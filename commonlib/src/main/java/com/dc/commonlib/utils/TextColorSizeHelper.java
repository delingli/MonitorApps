...
public class TextColorSizeHelper {
    ...
    public static class SpanInfo {
        ...
        private int subTextStrokeBgColor;
 ...
 /*       // 不展示背景圆角 不可点击
        ...
        // 展示背景圆角 不可点击
        public SpanInfo(String subText, int subTextSize, int subTextColor,
                        int subTextBgColor, int subTextStrokeBgColor, int subTextBgRadius,
                        boolean findFromEnd) {
            this(subText, subTextSize, subTextColor,
                    subTextBgColor, subTextStrokeBgColor, subTextBgRadius,
                    null, false, findFromEnd);
        }
        ...
        public SpanInfo(String subText, int subTextSize, int subTextColor,
            ...
            this.subTextStrokeBgColor = subTextStrokeBgColor;
    ...
    public static class RadiusBackgroundSpan extends ReplacementSpan {
         ...
         * @param radius        圆角半径
         */
        public RadiusBackgroundSpan(int textColor, int radiusBgColor, int mRadiusStrokeColor, int radius) {
            this.mTextColor = textColor;
            this.mRadiusBgColor = radiusBgColor;
            this.mRadius = radius;
            this.mRadiusStrokeColor = mRadiusStrokeColor;
        }
        ...
        public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
            ...
            Log.i("pyt", y + "");
            RectF oval = new RectF(x-6, y + paint.ascent()-3, x + mSize+7, y + paint.descent()+2);
            //设置文字背景矩形，x为span其实左上角相对整个TextView的x值，y为span左上角相对整个View的y值。paint.ascent()获得文字上边缘，paint.descent()获得文字下边缘
            canvas.drawRoundRect(oval, mRadius, mRadius, paint);//绘制圆角矩形，第二个参数是x半径，第三个参数是y半径
            paint.setStyle(Paint.Style.STROKE);
            paint.setColor(mRadiusStrokeColor);
