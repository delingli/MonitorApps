...
public class WebSocketWrapper {
    ...
    private volatile WebSocketClient mWebSocket;
            ...
            try {
                if (mWebSocket == null) {
                    ...
                    mWebSocket = new MyWebSocketClient(
                            ...
                            connectTimeOut);
                    WSLogUtil.i(TAG, "WebSocket start connect...");
                    ...
                    WSLogUtil.i(TAG, "WebSocket reconnecting...");
                ...
                connectStatus = 0;
                WSLogUtil.e(TAG, "WebSocket connect failed:", e);
    ...
    void disConnect() {
        needClose = true;
        if (connectStatus == 2) {
            WSLogUtil.i(TAG, "WebSocket disconnecting...");
            if (mWebSocket != null && !mWebSocket.isClosed()) {
                mWebSocket.close();
            }
            WSLogUtil.i(TAG, "WebSocket disconnected");
        }
    }
    ...
    void send(Request request) {
        ...
        if (request == null) {
            WSLogUtil.e(TAG, "send data is null!");
            return;
        }
        if (connectStatus == 2) {
            try {
                request.send(mWebSocket);
                WSLogUtil.i(TAG, "send success:" + request.toString());
            } catch (WebsocketNotConnectedException e) {
                connectStatus = 0;
                WSLogUtil.e(TAG, "ws is disconnected, send failed:" + request.toString(), e);
                if (mSocketListener != null) {
                    //not connect
                ...
                connectStatus = 0;
                WSLogUtil.e(TAG, "Exception,send failed:" + request.toString(), e);
                if (mSocketListener != null) {
                    //unknown error
                    mSocketListener.onSendDataError(request,
            ...
            WSLogUtil.e(TAG, "WebSocket not connect,send failed:" + request.toString());
            if (mSocketListener != null) {
                //not connect
                mSocketListener.onSendDataError(request,
                        ErrorResponse.ERROR_NO_CONNECT,
    ...
    private void checkDestroy() {
        if (destroyed) {
            try {
                ...
                WSLogUtil.e(TAG, "checkDestroy(WebSocketClient)", e);
            }
        }
    }
    ...
    private void onWSCallbackOpen(ServerHandshake handshakeData) {
        ...
        connectStatus = 2;
        WSLogUtil.i(TAG, "WebSocket connect success");
        if (needClose) {
            disConnect();
        } else {
            if (mSocketListener != null) {
    ...
    private void onWSCallbackMessage(String message) {
        ...
        if (mSocketListener != null) {
            ...
            WSLogUtil.i(TAG, "WebSocket received message:" + response.toString());
    ...
    private void onWSCallbackWebsocketPing(Framedata f) {
        ...
        if (mSocketListener != null) {
            ...
            WSLogUtil.i(TAG, "WebSocket received ping:" + response.toString());
            mSocketListener.onMessage(response);
        }
    }
    ...
    private void onWSCallbackWebsocketPong(Framedata f) {
        ...
        if (mSocketListener != null) {
            ...
            response.setResponseData(f);
            WSLogUtil.i(TAG, "WebSocket received pong:" + response.toString());
            mSocketListener.onMessage(response);
        }
    }
    ...
    private void onWSCallbackClose(int code, String reason, boolean remote) {
        connectStatus = 0;
        WSLogUtil.d(TAG, String.format("WebSocket closed!code=%s,reason:%s,remote:%s",
                code,
                reason,
                remote));
        if (mSocketListener != null) {
    ...
    private void onWSCallbackError(Exception ex) {
        ...
        WSLogUtil.e(TAG, "WebSocketClient#onError(Exception)", ex);
    }
