...
public class DefaultReconnectManager implements ReconnectManager {
    ...
    public void startReconnect() {
        if (reconnecting) {
            WSLogUtil.i(TAG, "Reconnecting, do not call again.");
            return;
        }
        if (destroyed) {
            WSLogUtil.e(TAG, "ReconnectManager is destroyed!!!");
            return;
        }
        ...
        try {
            ...
            WSLogUtil.e(TAG, "线程队列已满，无法执行此次任务。", e);
            reconnecting = false;
        }
    }
    ...
    private Runnable getReconnectRunnable() {
        return new Runnable() {
            ...
            public void run() {
                ...
                WSLogUtil.d(TAG, "开始重连:" + reconnectCount);
                reconnectCount++;
                reconnecting = true;
                connected = false;
                try {
                    ...
                    //重连失败
                    WSLogUtil.i(TAG, "reconnectOnce failed!");
                    mOnDisconnectListener.onDisconnect();
                } finally {
                    WSLogUtil.d(TAG, "重连结束:" + finishCount);
                    finishCount++;
                    reconnecting = false;
                    WSLogUtil.i(TAG, "reconnecting = false");
                }
            }
        };
    }
    ...
    public void onConnected() {
        connected = true;
        synchronized (BLOCK) {
            WSLogUtil.i(TAG, "onConnected()->BLOCK.notifyAll()");
            BLOCK.notifyAll();
        }
    }
    ...
    public void onConnectError(Throwable th) {
        connected = false;
        synchronized (BLOCK) {
            WSLogUtil.i(TAG, "onConnectError(Throwable)->BLOCK.notifyAll()");
            BLOCK.notifyAll();
        }
    }
