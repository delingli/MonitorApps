...
import com.zld.websocket.dispatcher.ResponseProcessEngine;
import com.zld.websocket.util.LogImpl;
import com.zld.websocket.util.WSLogUtil;
import com.zld.websocket.util.Logable;
import com.zld.websocket.util.PermissionUtil;
...
public class WebSocketHandler {
    ...
    public static WebSocketManager init(WebSocketSetting setting) {
        if (defaultWebSocket == null) {
            ...
            WSLogUtil.e(TAG, "Default WebSocketManager exists!do not start again!");
        }
    ...
    public static WebSocketManager initGeneralWebSocket(String key, WebSocketSetting setting) {
        ...
        synchronized (WS_MAP_BLOCK) {
            if (mWebSocketMap.containsKey(key)) {
                WSLogUtil.e(TAG, "WebSocketManager exists!do not start again!");
    ...
    public static void registerNetworkChangedReceiver(Context context) {
        if (PermissionUtil.checkPermission(context, Manifest.permission.ACCESS_NETWORK_STATE)) {
            try {
                ...
                WSLogUtil.e(TAG, "网络监听广播注册失败：", e);
            }
        } else {
            WSLogUtil.e(TAG, "未获取到网络状态权限，广播监听器无法注册");
        }
    }
